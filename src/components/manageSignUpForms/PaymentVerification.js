...
const PaymentVerification = () => {
  ...
  useEffect(() => {
    if (regState?.agentRegistrations !== null) {
      const { new_deposit, total_rows } = regState.agentRegistrations;

      setTotalRows(total_rows);
      setTableData(handleTableListData(new_deposit));
      setData(new_deposit);
    } else {
      setTableData([]);
      setTotalRows(0);
    ...
    <Fragment>
     ...
     "
      >
        ...
        {currentUser.roleDescription === "Account" && accountVerifyPaymenModal && (
          <ModalComponent>
